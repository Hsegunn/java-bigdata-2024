...
# all 현재 컬렉션 데이터의 조건 ,참인 값만 있는지 확인
...
## ★ enumerate() : 열거 , 데이터포함 인덱스를 같이 생성해주는 역할
...
## ★ map: 여러값을 한번에 같은 조건으로 변경
def ttime(x):
    return x*2

print(list(map(ttime, [1,3,5,7,9]))) # map을 써서 반복데이터를 ttime() 함수로 처리
print(list(map(int, [1.0,2.0,3.0,4.4]))) # map을 써서 반복데이터를 int로 변환
...
## round() 반올림(이상징후 4.5도 4로 나옴 >> 4.51부터 반올림됨)
print(round(4.51))
print(round(4.45, 1))
...
## 둘의 갯수가 일치하지 않으면 일치하는 값까지만 묶는다 (나머지 값은 버림)
odd = [1,3,5,7,9]
even = [2,4,6,8,10]
norm = [1,2,3,4,5]
