...
# QMainWindow, QLabel, QPushButton 등은 QWidget을 상속한 자식 클래스(부모 클래스의 능력을 다 사용가능)
...
class qtApp(QWidget): # QWidget이 가지고 있는 속성, 변수, 함수를 다 사용가능
    ...
    def initUI(self):
        label = QLabel() # 라벨위젯(PyQt), 라벨컨트롤(MFC, C#, Java Fx, Android)
        ...
        label.setStyleSheet(('color: violet;'
